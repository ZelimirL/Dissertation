...
The subsequent studies and textbooks on the $ H_2^{+} $ systems, rely on the Bates results. So any new approach for solving $ H_2^{+} $ should be able to replicate the Bates' results exactly. Wasserman in \cite{ExperimentalBates2} reports that while the existance of $ H_2^{+} $ has been verified, no  discreete spectrum othe $ H_2^{+} $ has been observed. However in the same thesis Wasserman reports that the ionization potential of $ H_2^{+} $ has been measured and that experimental methods show good agreement with Bates calculations.
...
The only place where we specifically are not using Atomic units will be the following derivation, where we do transformation of the molecular Hamiltonian into a Jacobian coordinate system. Since some terms will contain reduced mass, we want to keep the units intact.
...
\end{split}
\end{equation}
The second term in the expression for the $ H_{AD} $ represents the mass polarization term, and because $ M >> m $, it can be neglected. This term represents the movement of the electron as the center of the mass is moving. However, the center of the mass is moving much slower than the electron, due to the mass difference, and thus justifies neglecting this term.
TODO: Check mass polarization
This expression contains so called 'mass polarization'. The mass polarization refers to the 
https://books.google.com/books?id=jMicDQAAQBAJ&pg=PA92&lpg=PA92&dq=nuclei+mass+polarization&source=bl&ots=eKr1Ayj3t3&sig=q2gtkYmnjWv-9qdwZQ0kBew_krQ&hl=en&sa=X&ved=0ahUKEwidw_Ky8ITWAhVmqFQKHT2GCaAQ6AEIbDAJ#v=onepage&q=nuclei%20mass%20polarization&f=false
https://arxiv.org/pdf/0810.2210.pdf
http://iopscience.iop.org/article/10.1088/0953-4075/36/10/306/pdf
https://en.wikipedia.org/wiki/Molecular_Hamiltonian
https://physics.stackexchange.com/questions/72298/where-do-mass-polarization-terms-come-from-in-many-body-hamiltonian-why-are-the
...
\subsection{Series Expansions and Eigenvalue Determinant}
...
% This is done in a Mathematica script BatesRepro.nb
